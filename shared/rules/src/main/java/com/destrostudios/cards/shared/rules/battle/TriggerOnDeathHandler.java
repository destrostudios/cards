...
public class TriggerOnDeathHandler extends GameEventHandler<DestructionEvent> {
    ...
    public void handle(DestructionEvent event, NetworkRandom random) {
        ...
        for (int entity : data.query(Components.DEATH_TRIGGERS).list()) {
            TriggerUtil.trigger(data.getComponent(entity, Components.DEATH_TRIGGERS), entity, new int[] { event.target }, events, random);
        }
    }
}
