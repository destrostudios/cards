...
public class RemoveDefaultCastFromHandSpellBuffsOnRemovedFromHandHandler extends GameEventHandler<RemovedFromHandEvent> {
    ...
    public void handle(GameContext context, RemovedFromHandEvent event) {
        ...
        IntList buffs = data.getComponent(defaultCastFromHandSpell, Components.BUFFS);
        if (buffs != null) {
            LOG.debug("Removing all {} buffs from defaultCastFromHandSpell {} of {}", buffs.size(), inspect(data, defaultCastFromHandSpell), inspect(data, event.card));
            for (int buff : buffs) {
